...
export const ConvertExcelToImagesSchema = z.object({
...
export const ConvertWordToImagesSchema = z.object({
  filePath: z.string().describe("Path to the Word file (.docx, .doc)"),
  outputDir: z.string().optional().describe("Output directory for images"),
  dpi: z.number().int().min(72).max(600).optional().default(150).describe("Rendering DPI"),
  keepPdf: z.boolean().optional().default(false).describe("Keep intermediate PDF"),
});

export const ConvertPdfToImagesSchema = z.object({
...
export const ExtractWordTextSchema = z.object({
  filePath: z.string().describe("Path to the Word file (.docx)"),
  format: z.enum(["text", "markdown"]).default("markdown").describe("Output format"),
});

export const ConvertPdfImagesToMarkdownSchema = z.object({
  filePath: z.string().describe("Path to the PDF file"),
  dpi: z.number().int().min(72).max(600).optional().default(150).describe("Rendering DPI"),
